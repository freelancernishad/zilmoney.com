    ...
    public function login(LoginRequest $request)
    {
        ...
        if (Auth::attempt($credentials)) {
            ...
            if (is_null($user->email_verified_at)) {
                ...
                $emailSent = true;
                $emailMessage = 'Login successful. OTP has been sent to your email.';

                try {
                    Mail::to($user->email)->send(new OtpNotification($otp));
                } catch (\Exception $e) {
                    $emailSent = false;
                    $emailMessage = 'Login successful, but we could not send the OTP. Please try again later.';
                    Log::error('User Login OTP email failed: ' . $e->getMessage());
                    logUserActivity(
                        activity: 'Login OTP Email Failed',
                        category: 'Email',
                        userId: $user->id,
                        request: $request,
                        isSuccess: false,
                ...
                return response()->json(new LoginRegisterUserResource($user, $token, $emailMessage, !$emailSent), 200);
            }
